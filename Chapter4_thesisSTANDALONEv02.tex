...
The key goal of biofuels production is the optimisation of biomass productivity in large-scale microalgal culturing systems such as open ponds or closed photoÄêbioreactors.

At the broadest scale / biggest picture the growth of microalgae can be expressed as the following chemical reaction

\begin{align}
\ce{
 106 CO_2(g)  + 122 H_2O  + 16  NO_3^-(aq)  + PO_4^{3-}(aq)  + 19 H^+}\nonumber\\
 \ce{
  \reactrarrow{0pt}{2.5cm}{$\left[10^3-10^4\right]\gamma$}{} C_{106} H_{263} O_{110} N_{16} P_1 + 138 O_2 }(aq)
\end{align}

This equation captures a number of features that interest.  The total biomass is determined by the concentrations of \ce{N} and \ce{P}. A typical growth media for microalgae is  Guillard's Marine Enriched Seawater (F/2), which has 
\begin{table}
\begin{tabular}{ |c|r|}
...
Taking the limiting nutrient to be Nitrogen, the maximum biomass of algae is $200 mg L^{-1}$, of which about $70 mg L^{-1}$ is Carbon.  At Standard Temperature and Pressure, this is would be $0.12 L$ of pure \ce{CO_2} which is equivalent to $325 L$ of air for each litre of media.
