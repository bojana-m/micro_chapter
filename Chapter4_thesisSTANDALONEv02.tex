...
\textbf{In this run:}\\
$P_1$, $R_1$, $kLA_{O_2}$, $K_m$, $R_R$, $RQ_d$, and $RQ_n$ are all treated as parameters constant through time but unknown with prior distributions and proposal distributions defined in Table \ref{tab:micro_sim_priors}. The other model parameters were defined earlier in Table \ref{tab:micro_parameter_model}.
The data model assigned log normally distributed observation errors for each instrument (Section \ref{sec:micro_data_model}).
...
\textbf{In this run:}\\
$kLA_{O_2}$, $K_m$, $R_R$, $RQ_d$, and $RQ_n$ are all treated as parameters constant through time but unknown with prior distributions and proposal distributions defined in Table \ref{tab:micro_sim_priors2}.
...
\FloatBarrier
\subsection{Posterior results with experimental data where photosynthesis and respiration are changing through time, and respiratory quotients are changing through time with bounds}
% micro_iterative.bi
...
\FloatBarrier
\subsection{Posterior results with experimental data where photosynthesis, respiration and respiratory quotients are changing through time and an offest on $O_2$ is introduced}
