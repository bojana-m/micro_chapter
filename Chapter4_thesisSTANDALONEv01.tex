...
Km 	~ log_normal(log(100.0), 0.2)
kLAO2 	~ log_normal(log(200.0), 0.5)

sigma_r_R 	~ normal(0.01, 0.001)
sigma_r_P 	~ normal(0.05, 0.005)
}

const prop_std = 0.1;
sub proposal_parameter {
Km 	~ log_normal(log(Km), 0.2*prop_std)
kLAO2 	~ log_normal(log(kLAO2), 0.5*prop_std)

sigma_r_R 	~ normal(sigma_r_R, 0.001*prop_std)
sigma_r_P 	~ normal(sigma_r_P, 0.005*prop_std)
}

sub initial {/* prior distribution over initial conditions, given parameters */
// specify the initial condition model 
R 	~ normal(log(1000.0), 0.3)
R1 	~ log_normal(log(1000.0), 0.3)
P 	~ normal(log(30000.0), 0.8)
P1 	~ log_normal(log(30000.0), 0.8)
alk 	~ log_normal(log(1700.0), 0.05)

DIC 	~ log_normal(log(1500.0), 0.1) 
O_2 	~ log_normal(log(230.0), 0.2)   
pH 	~ log_normal(log(8.5), 0.2)
CO2 	~ log_normal(log(5.0), 0.1)
HCO3 	~ log_normal(log(1300.0), 0.1)
CO3 	~ log_normal(log(200.0), 0.1)
O_2H 	~ log_normal(log(200.0), 0.1)	
CO2H 	~ log_normal(log(10.0), 0.1)
}
