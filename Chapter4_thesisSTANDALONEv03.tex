...
\begin{figure}
	...
	\caption[.]{Priors (pink) and posteriors (purple) for model parameters.}
	\label{fig:micro_offset_thinned_parameters_model1}
\end{figure}
...
There is not much difference between posteriors for $DIC$, $TA$, and $C_p$ (Figure \ref{fig:micro_offset_thinned_DIC_TA_Cp}) apart from the $C_p$ 95\% credible interval being wider when there are less $O_2$ observations.

carbon chemistry looks similar (Figure \ref{fig:micro_offset_thinned_carbon})

$P_1$ behaved similarly, with more smoothness through the median and slightly wider credible intervals when less $O-@$ observations were assimilated. $R_1$ notably was centred around 15$\mskip3mu$$\mu$M L$^{-1}$ hour$^{-1}$ with 95\% credible intervals 5-30$\mskip3mu$$\mu$M L$^{-1}$ hour$^{-1}$ (Figure \ref{fig:micro_offset_thinned_P_and_R}) compared to 9$\mskip3mu$$\mu$M L$^{-1}$ hour$^{-1}$ with 95\% credible intervals between 4$\mskip3mu$$\mu$M L$^{-1}$ hour$^{-1}$ and 20$\mskip3mu$$\mu$M L$^{-1}$ hour$^{-1}$ (Figure \ref{fig:micro_exp_offset_P_R}). 
...
parameters (Figure \ref{fig:micro_offset_thinned_parameters_model})

Parameter traces improved their mixing (Figure \ref{fig:micro_offset_thinned_parameters_model_trace}) from Section \ref{sec:micro_exp_offset}, but still displayed some correlation between samples which is not ideal.
...
After thinning out the $O_2$ observations further, mixing improved in Section \ref{sec:micro_exp_offset_thinned} but still was not ideal. Correlated samples were present [EJ: how do I address this issue?].
